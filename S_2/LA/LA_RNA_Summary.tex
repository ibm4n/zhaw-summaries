...
\begin{multicols*}{3}
    ...
    \WhiteSpace
    \includegraphics[scale=0.4]{Zwischenwinkel.png}
    ...
    {\circled{1} Ebenen in die Koordinatendarstellung schreiben\\
    \circled{2} 'd' auf die andere Seite des Gleicheitszeichen schreiben\\
    \circled{3} Lgs aufstellen\\
    \circled{4} Gauss-Jordan anwenden $\rightarrow$ Schnittgerade (siehe 'Lösung nach reduzierter Zeilenstufenform')\\
    Bsp:}
    \includegraphics[scale=0.3]{Schnittgerade.png}


    \subsection{ Schnittpunkt mit Gerade}
    {Aufpunkt von Ebene $\vec{Ea}$\\
    Aufpunkt von Gerade $\vec{ga}$\\
    $\vec{ga} - \vec{Ea} = \vec{c}$}
    \circled{1} Erste 2 Vektoren im Lgs sind die Parameter Vektoren der Ebene.
    Der Dritte Vektor, ist der Parameter vektor der geraden und der Lösungsvektor ist $\vec{c}$ \\
    \circled{2} Lgs mit Gauss Lösen \\
    \circled{3} Wenn die unterste Zeile nur noch aus einer führenden 1 in der untersten Zeie und einer Zahl im Lösungsvektor besteht, muss diese Zahl nur noch als Parameter in der Gerade eingesetzt werden.\\
    \WhiteSpace
    Sollte das Lgs nicht Lösbar sein, existiert kein Schnittpunkt $\vec{S}$
    \WhiteSpace
    ...
    \subsection{Lösung nach reduzierter Zeilenstufenform}
    {Bestimmen der Lösung nach reduzierter Zeilenstufenform}
    \includegraphics[scale=0.45]{Lgs_Loesung_nach_Gauss.png}
    ...
    \subsection{Matrizen als Vektor darstellen}

    $\begin{pmatrix} a & b \\ c & d \end{pmatrix} \quad
        \rightarrow \quad
        \begin{pmatrix} a \\ b \\ c \\ d \end{pmatrix}
    $\\
    \includegraphics[scale=0.4]{Matrices_to_vectors.png} $\rightarrow$
    \includegraphics[scale=0.4]{Matrices_to_vectors_2.png}
    ...
    \subsection{ Abbildungsmatrix cAb ermitteln }
    {\includegraphics[scale=0.6]{Abbildungsmatrix_cAb_ermitteln.png}}
    ...
    {f zuerst $\to g(f(x)) \to g \circ f \to B \cdot A$  }

    {g zuerst $\to f(g(x)) \to f \circ g \to A \cdot B$  }
